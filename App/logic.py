...
def new_logic():
    ...
    catalog['books_by_year_author'] = lp.new_map(500, 2)
...
def get_book_info_by_book_id(catalog, good_reads_book_id):
    ...
    """
    x = lp.get(catalog['books_by_id'], good_reads_book_id)
...
def get_books_by_author_pub_year(catalog, author_name, pub_year):
    ...
    author_books = lp.get(catalog['books_by_year_author'], author_name)
    
    if author_books:
        books_in_year = lp.get(author_books, pub_year)
        if books_in_year:
